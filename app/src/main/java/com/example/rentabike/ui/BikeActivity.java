...
public class BikeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bikeAdapter.setOnItemClickListener(new BikeAdapter.OnItemClickListener() {
            ...
            public void onEditClick(BikeEntity bike) {
                ...
                intent.putExtra(EditBikeActivity.EDIT_EXTRA_PICTURE, bike.getBikePicture());
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == ADD_BIKE_REQUEST && resultCode == RESULT_OK){
            String name = data.getStringExtra(AddBikeActivity.EXTRA_NAME);
            String description = data.getStringExtra(AddBikeActivity.EXTRA_DESCRIPTION);
            String size = data.getStringExtra(AddBikeActivity.EXTRA_SIZE);
            byte[]picture = data.getByteArrayExtra(AddBikeActivity.EXTRA_PICTURE);
            BikeEntity bike = new BikeEntity(name, description,size, picture);
            bikeListViewModel.insert(bike);
            Toast.makeText(this, "Bike added", Toast.LENGTH_SHORT).show();
        }
        else
            if (requestCode == EDIT_BIKE_REQUEST && resultCode == RESULT_OK) {
                int idBike = data.getIntExtra(EditBikeActivity.EDIT_EXTRA_ID, -1);
                ...
                byte[] picture = data.getByteArrayExtra((EditBikeActivity.EDIT_EXTRA_PICTURE));

                BikeEntity bikeEntity = new BikeEntity(name, description, size, picture);


                bikeListViewModel.update(bikeEntity);
