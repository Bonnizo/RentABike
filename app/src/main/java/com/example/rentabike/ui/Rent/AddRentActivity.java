...
public class AddRentActivity extends AppCompatActivity {
    ...
    private void saveRent() {
        ...
        int idBike  = getIntent().getIntExtra(RENT_EXTRA_ID_BIKE,-1);
        AppDatabase appDb = AppDatabase.getInstance(this);
        RentEntity rentEntity = appDb.rentDao().findRentExist(date, idBike);
        ...
        RentEntity rentEntityToAdd = new RentEntity(firstname,lastname,date,number,idBike);
        appDb.rentDao().insert(rentEntityToAdd);

        Intent intent = new Intent(AddRentActivity.this, BikeActivity.class);
